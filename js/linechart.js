...
async function getAverages(metric, train) {
    const average = arr => {
        console.log(arr);
        const sum = arr.reduce((acc, c) => {return acc + c});
        console.log(sum);
    ...
    return {
        control: average(controlData[property]),
        mlp: average(mlpData[property]),
        mlr: average(mlrData[property])
    }    
}

async function createBarChart(metric, train) {
    data = await getAverages(metric, train)

    const chartId = `${metric}-${train ? 'train' : 'test'}`
    console.log(chartId)
    const barChart = document.getElementById(chartId);

    const xLabels = ['Control', 'Multilayer Perceptron', 'Multiple Linear Regression']
    const yValues = [data.control, data.mlp, data.mlr];
    console.log(yValues)
...
async function createChart(metric, train) {    // metric can be 'mae' or 'r2', train can be true or false
    ...
    console.log(controlData[property])
...
createBarChart('mae', true);
createBarChart('mae', false);
createBarChart('r2', true);
createBarChart('r2', false);
